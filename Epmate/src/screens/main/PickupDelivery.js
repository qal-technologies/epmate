...
const PickupDelivery = ({ setIsSearching }) => {
    ...
    try {
      await fetch('/.netlify/functions/createErrand', {
        ...
        body: JSON.stringify({
          pickupLocation,
          deliveryLocation,
          userId: '123', // This would be the actual user ID
        }),
      });
    } catch (error) {
    ...
    <Provider>
      <View style={styles.container}>
        ...
        <TextInput
          ...
          style={styles.input}
        />
        <Button mode="contained" disabled={!isButtonEnabled} onPress={findHelper}>
          Find Helper
        </Button>
        <SearchingHelpersModal visible={isSearching} />
        <HelperListModal visible={isHelperListVisible} onDismiss={hideHelperList} />
      </View>
    </Provider>
  );
};
