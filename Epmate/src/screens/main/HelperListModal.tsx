...
const HelperListModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if(visible) {
      const helpersWithVisibility: HelperWithVisibility[] = helpers.map((helper, index) => ({
        ...helper,
        isVisible: false,
    ...
    const timer = setInterval(() => {
      setCountdown(prev => {
        if(prev <= 0) {
          setIsDisabled(true);
          clearInterval(timer);
          // Remove item after expiry - call immediately to avoid timeout leak
          onExpired(item.id);
          return 0;
        }
        return prev - 1;
